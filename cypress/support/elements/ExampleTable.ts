...
export default class ExampleTable {
  ...
  private readonly rowLocator: string =
    '//div[contains(@class,"policy-list-container")]/div[@class="data-container"]/div[contains(@class,"-row")][./*[%s and .="%s"]]';

  public getRowDataByColumnHeader(term: string): Chainable<any> {
    const rowLocator = util.format(this.rowLocator, ExampleTable.COLUMNS.columnHeader.position, term);
    ...
    cy.xpath(rowLocator).each((ele, index, $list) => {
      ...
      Object.keys(ExampleTable.COLUMNS).forEach((column) => {
        customerRecord[ExampleTable.COLUMNS[column]["name"]] = Cypress.$(ele)
          .find(util.format(" > *:nth-child(%s)", ExampleTable.COLUMNS[column]["position"]))
          .text()
          .trim();
      });
  ...
  public clickTextInColumnHeader(text: any): void {
    const rowLocator = util.format(this.rowLocator, ExampleTable.COLUMNS.columnHeader.position, text);
    cy.xpath(rowLocator)
      .first()
      .within(() => {
